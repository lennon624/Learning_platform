<template>
  <div class="page-banner" style="min-height: 420px;">
    ...
    <div id="container">
      ...
      <label for="tab-1">关于</label>
      <!--Pestaña 2 inactiva por defecto-->
      <input id="tab-2" type="radio" name="tab-group" />
      <label for="tab-2">课程评价</label>
...
export default {
      ...
      items: [
        {
          ...
          title: "课程名字",
          author: "教师",
          abs: "djsfkgdhjgjfsgdfsjkdhgfsjhdkfgsjkdfgskdf"
        }
      ],
  ...
  methods: {
    ...
    //取消选课
    removecourse(){
removeCourseFromList( {student_id : this.myform.studentId , course_id :  this.myform.courseId } )
        .then(res => {
          console.log(res);
            this.toast("取消选课成功");
        })
...
addCourse2List( {student_id : this.myform.studentId , course_id :  this.myform.courseId } )
        .then(res => {
          console.log(res);
        })
        .catch(err => {
    ...
    goCourseDetails(){
      let study = this.course.id
        ...
        this.navigator("/index/"+this.form.parent_category_id+"/"+this.$route.params.course+"/study");
    },
